...
def _extract_excerpt(fp: Path) -> str:
    """Extract the first ~200 words from a file."""
    if fp.suffix.lower() == ".pdf":
        return _extract_pdf_excerpt(fp)
    ...
    return _truncate_words(text, _MAX_WORDS_PER_DOC)


def _extract_pdf_excerpt(fp: Path) -> str:
    ...
    try:
        with pdfplumber.open(fp) as pdf:
            ...
            for page in pdf.pages:
                ...
                if sum(len(t.split()) for t in text_parts) >= _MAX_WORDS_PER_DOC:
                    break
            return _truncate_words(" ".join(text_parts), _MAX_WORDS_PER_DOC)
    except Exception:
