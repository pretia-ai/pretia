...
def run(
    ...
    try:
        ...
        if not no_html:
            try:
                from agentcost.report import render_and_save
...
def _show_confirmation(workflow_path: str, num_runs: int) -> None:
    ...
    est_cost = "unknown"
    try:
        from agentcost.estimate import estimate_workflow
...
def recommend_cmd(profile_path: str) -> None:
    ...
    from agentcost.ci.report import (
...
def estimate_cmd(workflow_path: str, traffic: int | None, verbose: bool) -> None:
    ...
    if est.models:
        ...
        for m in est.models:
            ...
            if m.canonical_name:
                try:
                    from agentcost.pricing.tables import model_tier
    ...
    console.print(
        ...
        "Run [bold]agentcost profile run[/bold] for distributional "
