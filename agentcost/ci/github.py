...
from agentcost.ci.baseline import Baseline, load_baseline
from agentcost.ci.report import format_cost
from agentcost.estimate import WorkflowEstimate, estimate_workflow
from agentcost.recommend.base import Recommendation
from agentcost.recommend.score import OptimizationScore

if TYPE_CHECKING:
    from agentcost.ci.diff import DiffResult

logger = logging.getLogger(__name__)

_COMMENT_MARKER = "<!-- agentcost-pr-comment -->"
...
def format_diff_only_comment(
    ...
    lines.append(f"## {_DEFAULT_EMOJI} AgentCost — Cost Estimate")
    lines.append("")
    ...
    if has_baseline:
        ...
        detail_lines.append("")
        detail_lines.append(
            "*No baseline found. Run `agentcost baseline update` to enable delta tracking.*"
        )
    ...
    lines.append(
        ...
        "[Powered by AgentCost](https://github.com/agentcost/agentcost)</sub>"
    )
...
def format_full_profile_comment(
    ...
    lines: list[str] = [_COMMENT_MARKER]
    lines.append(f"## {emoji} AgentCost — {score.score}/100 ({score.zone_label})")
    lines.append("")
    ...
    footer_parts.append("[Powered by AgentCost](https://github.com/agentcost/agentcost)")
...
def run_full_profile(
    ...
    from agentcost.recommend import compute_score, generate_recommendations
    from agentcost.report.renderer import render_and_save
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="AgentCost GitHub Action")
    parser.add_argument("--workflow-path", required=True)
    parser.add_argument("--baseline-path", default=".agentcost/baseline.json")
