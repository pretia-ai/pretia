...
def load_corpus(corpus_path: str) -> Corpus:
    ...
    if not path.exists() or path.is_dir():
        if path.is_dir():
            # Input specifies a PDF directory, not an embeddings file.
            # Check for an embeddings.json inside it or fall back to mock.
            embedded = path / "embeddings.json"
            if embedded.exists():
                path = embedded
            else:
                logger.warning(
                    "Corpus path %s is a directory without embeddings.json "
                    "— returning mock corpus. Run scripts/build_rag_corpus.py "
                    "to build the embedding index.",
                    path,
                )
